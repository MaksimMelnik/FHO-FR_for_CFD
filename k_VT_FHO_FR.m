function k_VT=k_VT_FHO_FR(T, M1, M2, i1, f1, method)
...
% f1 is the final vibrational level, method is integration method
% ('q' for integralN, 't' for trapz).
...
if M2.num_vibr_levels(1)==1
    switch method
        case 'q'
            int=integral(@(E) (E./T_cm1).^2 .* ...
                P_VT_FHO_FR_avg_angles(M1, M2, i1, f1, E*h*c*100, 't')...
                    .* exp(-E./T_cm1)/T_cm1, 0, Inf, ...
...
%             error('still not implemented')
            ...
            Emin=1e0; Emax=1e7;
            fr_num=250;
            E_arr=10.^(log10(Emin):(log10(Emax)-log10(Emin))/fr_num:...
                                                            log10(Emax));
            temp=(E_arr./T_cm1).^2 .* P_VT_FHO_FR_avg_angles(M1, M2, ...
                i1, f1, E_arr*h*c*100, 't') .* exp(-E_arr./T_cm1)/T_cm1;
            int=trapz(E_arr, temp);
    end
else

function pVT=P_VT_FHO_FR_MM_ij_trapz(M1, M2, i1, f1, E)
...
% Diatom-diatom collision. M1 is the first molecule, M2 is the second
% molecule, Coll is the collision variable, i1 is the initial vibrational
% state of M1, f1 is the final vibrational state of M1, E is the
% collision energy (J).
...
pVT=integral2(@(eps1, eps2) ... integration over eps via integral2
        integrant_trapz(M1, M2, i1, f1, E, eps1, eps2), ...
                    0,1, 0,eps2max, 'RelTol',error_v,'AbsTol',error_v);
pVT=pVT/pi^4;
end

function out=integrant_trapz(M1, M2, i1, f1, E, eps1, eps2)
...
out=P_VT_FHO_FR_MM_ij(M1, M2, i1, f1, ...

function pvt=P_VT_FHO_FR_avg_angles(M1, M2, i1, f1, E, method)
...
% (may be a vector), method is integration method ('q' for integralN, 
% 't' for trapz).
...
switch method
    case 'q'
        ...
        pvt=integralN(@(y, eps1, theta_v1, phi1) ...
	        P_VT_FHO_FR_MA_ij(M1, M2, i1, f1, E, eps1, y, ...
                                                    theta_v1, phi1), ...
    ...
    case 't'
        ...
        E_temp =    reshape(E,          1, 1, 1, 1, []);    % 5
        out=P_VT_FHO_FR_MA_ij(M1, M2, i1, f1, E_temp, eps1rs, ...
                                                yrs, theta_v1rs, phi1rs);
        pvt=trapz(phi1, trapz(theta_v1, trapz(y, ...
                                        trapz(eps1, out, 1), 2), 3), 4);
        pvt=reshape(pvt, 1, [], 1, 1, 1);
end
pvt=pvt/pi^2;
end
